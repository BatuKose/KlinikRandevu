// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Repositories.EFCore;

#nullable disable

namespace KlinikRandevu.Migrations
{
    [DbContext(typeof(RepositoryContext))]
    [Migration("20260912182011_PoliklinikYesilAlanAyarlarıUzKod")]
    partial class PoliklinikYesilAlanAyarlarıUzKod
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.26")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Entities.Models.Doctor", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("DoktorAd")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ServisNo")
                        .HasColumnType("int");

                    b.Property<int>("doktorNo")
                        .HasColumnType("int");

                    b.Property<long>("doktorTc")
                        .HasColumnType("bigint");

                    b.Property<int>("doktorUzKod")
                        .HasColumnType("int");

                    b.Property<bool>("isActive")
                        .HasColumnType("bit");

                    b.Property<int>("tescilNO")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("doktorNo")
                        .IsUnique();

                    b.HasIndex("doktorTc")
                        .IsUnique();

                    b.ToTable("Doktorlar", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            DoktorAd = "HÜSEYİN AĞAC",
                            ServisNo = 1,
                            doktorNo = 1,
                            doktorTc = 11111111111L,
                            doktorUzKod = 1000,
                            isActive = true,
                            tescilNO = 10001
                        },
                        new
                        {
                            Id = 2,
                            DoktorAd = "AYŞE DEMİR",
                            ServisNo = 2,
                            doktorNo = 2,
                            doktorTc = 22222222222L,
                            doktorUzKod = 1100,
                            isActive = true,
                            tescilNO = 10002
                        },
                        new
                        {
                            Id = 3,
                            DoktorAd = "MEHMET YILMAZ",
                            ServisNo = 3,
                            doktorNo = 3,
                            doktorTc = 33333333333L,
                            doktorUzKod = 1300,
                            isActive = true,
                            tescilNO = 10003
                        });
                });

            modelBuilder.Entity("Entities.Models.DoktorCalismaPlani", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<TimeSpan>("BaslangicSaati")
                        .HasColumnType("time");

                    b.Property<TimeSpan>("BitisSaati")
                        .HasColumnType("time");

                    b.Property<int>("DoktorNo")
                        .HasColumnType("int");

                    b.Property<int>("GunAdi")
                        .HasColumnType("int");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<int>("PolNo")
                        .HasColumnType("int");

                    b.Property<int>("RandevuSuresiDk")
                        .HasColumnType("int");

                    b.Property<bool>("YesilAlanZorunlu")
                        .HasColumnType("bit");

                    b.HasKey("Id");

                    b.ToTable("CalismaPlanlari", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            BaslangicSaati = new TimeSpan(0, 8, 0, 0, 0),
                            BitisSaati = new TimeSpan(0, 12, 0, 0, 0),
                            DoktorNo = 1,
                            GunAdi = 1,
                            IsActive = true,
                            PolNo = 1,
                            RandevuSuresiDk = 10,
                            YesilAlanZorunlu = false
                        },
                        new
                        {
                            Id = 2,
                            BaslangicSaati = new TimeSpan(0, 8, 0, 0, 0),
                            BitisSaati = new TimeSpan(0, 12, 0, 0, 0),
                            DoktorNo = 1,
                            GunAdi = 3,
                            IsActive = true,
                            PolNo = 1,
                            RandevuSuresiDk = 10,
                            YesilAlanZorunlu = false
                        },
                        new
                        {
                            Id = 3,
                            BaslangicSaati = new TimeSpan(0, 8, 0, 0, 0),
                            BitisSaati = new TimeSpan(0, 12, 0, 0, 0),
                            DoktorNo = 1,
                            GunAdi = 5,
                            IsActive = true,
                            PolNo = 1,
                            RandevuSuresiDk = 10,
                            YesilAlanZorunlu = false
                        },
                        new
                        {
                            Id = 4,
                            BaslangicSaati = new TimeSpan(0, 9, 0, 0, 0),
                            BitisSaati = new TimeSpan(0, 17, 0, 0, 0),
                            DoktorNo = 2,
                            GunAdi = 2,
                            IsActive = true,
                            PolNo = 2,
                            RandevuSuresiDk = 15,
                            YesilAlanZorunlu = false
                        },
                        new
                        {
                            Id = 5,
                            BaslangicSaati = new TimeSpan(0, 9, 0, 0, 0),
                            BitisSaati = new TimeSpan(0, 17, 0, 0, 0),
                            DoktorNo = 2,
                            GunAdi = 4,
                            IsActive = true,
                            PolNo = 2,
                            RandevuSuresiDk = 15,
                            YesilAlanZorunlu = false
                        },
                        new
                        {
                            Id = 6,
                            BaslangicSaati = new TimeSpan(0, 10, 0, 0, 0),
                            BitisSaati = new TimeSpan(0, 16, 0, 0, 0),
                            DoktorNo = 3,
                            GunAdi = 1,
                            IsActive = true,
                            PolNo = 3,
                            RandevuSuresiDk = 20,
                            YesilAlanZorunlu = false
                        },
                        new
                        {
                            Id = 7,
                            BaslangicSaati = new TimeSpan(0, 10, 0, 0, 0),
                            BitisSaati = new TimeSpan(0, 16, 0, 0, 0),
                            DoktorNo = 3,
                            GunAdi = 2,
                            IsActive = true,
                            PolNo = 3,
                            RandevuSuresiDk = 20,
                            YesilAlanZorunlu = false
                        },
                        new
                        {
                            Id = 8,
                            BaslangicSaati = new TimeSpan(0, 10, 0, 0, 0),
                            BitisSaati = new TimeSpan(0, 16, 0, 0, 0),
                            DoktorNo = 3,
                            GunAdi = 3,
                            IsActive = true,
                            PolNo = 3,
                            RandevuSuresiDk = 20,
                            YesilAlanZorunlu = false
                        },
                        new
                        {
                            Id = 9,
                            BaslangicSaati = new TimeSpan(0, 10, 0, 0, 0),
                            BitisSaati = new TimeSpan(0, 16, 0, 0, 0),
                            DoktorNo = 3,
                            GunAdi = 4,
                            IsActive = true,
                            PolNo = 3,
                            RandevuSuresiDk = 20,
                            YesilAlanZorunlu = false
                        },
                        new
                        {
                            Id = 10,
                            BaslangicSaati = new TimeSpan(0, 10, 0, 0, 0),
                            BitisSaati = new TimeSpan(0, 16, 0, 0, 0),
                            DoktorNo = 3,
                            GunAdi = 5,
                            IsActive = true,
                            PolNo = 3,
                            RandevuSuresiDk = 20,
                            YesilAlanZorunlu = false
                        });
                });

            modelBuilder.Entity("Entities.Models.DoktorIzın", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Aciklama")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("DoktorNo")
                        .HasColumnType("int");

                    b.Property<bool>("Iptal")
                        .HasColumnType("bit");

                    b.Property<DateOnly>("IzinBaslangic")
                        .HasColumnType("date");

                    b.Property<DateOnly>("IzinBitis")
                        .HasColumnType("date");

                    b.HasKey("Id");

                    b.ToTable("DoktorIzın");
                });

            modelBuilder.Entity("Entities.Models.HastaYesilListe", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("EklenmeTarihi")
                        .HasColumnType("datetime");

                    b.Property<long>("EkleyenDoktorTC")
                        .HasColumnType("bigint");

                    b.Property<int>("PolUzKod")
                        .HasColumnType("int");

                    b.Property<bool>("aktifMi")
                        .HasColumnType("bit");

                    b.Property<long>("hastaTc")
                        .HasColumnType("bigint");

                    b.Property<int>("muayeneId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("HastaYesilListe");
                });

            modelBuilder.Entity("Entities.Models.IcdApiEntegrasyon", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("GecerlilikSüresi")
                        .HasColumnType("datetime");

                    b.Property<string>("Token")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TokenType")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("IcdApiEntegrasyon");
                });

            modelBuilder.Entity("Entities.Models.MuayeneKaydi", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<TimeSpan>("BaslangicSaati")
                        .HasColumnType("time");

                    b.Property<TimeSpan?>("BitisSaati")
                        .HasColumnType("time");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime");

                    b.Property<int>("DoktorNo")
                        .HasColumnType("int");

                    b.Property<long>("HastaTc")
                        .HasColumnType("bigint");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<DateTime>("MuayeneTarihi")
                        .HasColumnType("datetime");

                    b.Property<int>("PolNo")
                        .HasColumnType("int");

                    b.Property<int>("ProtocolNo")
                        .HasColumnType("int");

                    b.Property<int?>("RandevuId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("MuayeneKayitlari", (string)null);
                });

            modelBuilder.Entity("Entities.Models.Patient", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime>("BirthDate")
                        .HasColumnType("datetime");

                    b.Property<int>("BloodType")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Gender")
                        .HasColumnType("int");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int>("Protocol")
                        .HasColumnType("int");

                    b.Property<string>("Surname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<long>("TcKimlik")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.ToTable("Patients");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Address = "BURSA, Türkiye",
                            BirthDate = new DateTime(2001, 2, 21, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            BloodType = 6,
                            CreatedAt = new DateTime(2026, 9, 12, 21, 20, 10, 985, DateTimeKind.Local).AddTicks(7772),
                            Gender = 2,
                            IsActive = true,
                            Name = "BATUHAN",
                            Phone = "5378102935",
                            Protocol = 20261,
                            Surname = "KÖSE",
                            TcKimlik = 11111111111L
                        });
                });

            modelBuilder.Entity("Entities.Models.Poliklinik", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Aciklama")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<int>("DoktorNo")
                        .HasColumnType("int");

                    b.Property<int?>("GunlukMaksRandevuSayisi")
                        .HasColumnType("int");

                    b.Property<int?>("KatNo")
                        .HasColumnType("int");

                    b.Property<int?>("MaxRandevuSuresi")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("OdaNo")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<bool>("OnlineRandevuAktif")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<int>("PolNo")
                        .HasColumnType("int");

                    b.Property<int>("PolUzKod")
                        .HasColumnType("int");

                    b.Property<string>("Telefon")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<bool>("isActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.HasKey("Id");

                    b.HasIndex("PolNo")
                        .IsUnique();

                    b.ToTable("Poliklinikler", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Aciklama = "İÇ Hastalıkları pol öğleden önce",
                            DoktorNo = 1,
                            GunlukMaksRandevuSayisi = 30,
                            KatNo = 1,
                            MaxRandevuSuresi = 10,
                            Name = "İÇ HASTALIKLARI HÜSEYİN AĞAC",
                            OdaNo = "Birinci kat",
                            OnlineRandevuAktif = true,
                            PolNo = 1,
                            PolUzKod = 1000,
                            Telefon = "1650",
                            isActive = true
                        });
                });

            modelBuilder.Entity("Entities.Models.PoliklinikYesilAlanAyarları", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("PolUzKod")
                        .HasColumnType("int");

                    b.Property<int>("gecerlilikSüresi")
                        .HasColumnType("int");

                    b.Property<int>("polNo")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("PoliklinikYesilAlanAyarları");
                });

            modelBuilder.Entity("Entities.Models.Randevu", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime");

                    b.Property<int>("DoktorNo")
                        .HasColumnType("int");

                    b.Property<long>("HastaTc")
                        .HasColumnType("bigint");

                    b.Property<string>("Notlar")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<int>("PolNo")
                        .HasColumnType("int");

                    b.Property<int>("ProtocolNo")
                        .HasColumnType("int");

                    b.Property<bool>("RandevuOzelMesajGonderildi")
                        .HasColumnType("bit");

                    b.Property<DateTime>("RandevuTarihi")
                        .HasColumnType("datetime");

                    b.Property<int>("SureDakika")
                        .HasColumnType("int");

                    b.Property<bool>("hatirlatmaMailiGonderildi")
                        .HasColumnType("bit");

                    b.Property<bool>("iptal")
                        .HasColumnType("bit");

                    b.HasKey("Id");

                    b.HasIndex("DoktorNo", "RandevuTarihi")
                        .IsUnique();

                    b.ToTable("Randevular", (string)null);
                });

            modelBuilder.Entity("Entities.Models.RandevuBekleyenHastalar", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool>("Bilgilendirme")
                        .HasColumnType("bit");

                    b.Property<DateTime>("RandevuTarihi")
                        .HasColumnType("datetime");

                    b.Property<bool>("RandevuVerildi")
                        .HasColumnType("bit");

                    b.Property<int>("doktorNo")
                        .HasColumnType("int");

                    b.Property<int>("polNo")
                        .HasColumnType("int");

                    b.Property<int>("protokol")
                        .HasColumnType("int");

                    b.Property<string>("randevuNotu")
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("tcKimlik")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.ToTable("RandevuBekleyenHastalar");
                });

            modelBuilder.Entity("Entities.Models.SistemParametresi", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Aciklama")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<bool>("Aktif")
                        .HasColumnType("bit");

                    b.Property<string>("Deger1")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Deger2")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Deger3")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Deger4")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Deger5")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<DateTime?>("GuncellemeTarihi")
                        .HasColumnType("datetime");

                    b.Property<DateTime>("OlusturmaTarihi")
                        .HasColumnType("datetime");

                    b.Property<string>("ParametreAdi")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id");

                    b.HasIndex("ParametreAdi")
                        .IsUnique();

                    b.ToTable("parametreler");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Aciklama = "D1: Aktif mi (EVET/HAYIR), D2: Hata mesajı",
                            Aktif = true,
                            Deger1 = "EVET",
                            OlusturmaTarihi = new DateTime(2026, 9, 12, 21, 20, 10, 986, DateTimeKind.Local).AddTicks(7960),
                            ParametreAdi = "KADIN_DOGUM_ERKEK_YASAKLA"
                        },
                        new
                        {
                            Id = 2,
                            Aciklama = "D1: Aktif mi, D2: Min yaş, D3: Max yaş",
                            Aktif = true,
                            Deger1 = "EVET",
                            Deger2 = "0",
                            Deger3 = "16",
                            OlusturmaTarihi = new DateTime(2026, 9, 12, 21, 20, 10, 986, DateTimeKind.Local).AddTicks(7970),
                            ParametreAdi = "PEDIATRI_YAS_LIMITI"
                        });
                });

            modelBuilder.Entity("Entities.Models.Taahütname", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool>("BilgilendirmeMail")
                        .HasColumnType("bit");

                    b.Property<bool>("BilgilendirmeSms")
                        .HasColumnType("bit");

                    b.Property<int>("MuayeneId")
                        .HasColumnType("int");

                    b.Property<DateTime>("SonOdemeTarihi")
                        .HasColumnType("datetime");

                    b.Property<DateTime>("TahütTarihi")
                        .HasColumnType("datetime");

                    b.Property<double>("ToplamBorc")
                        .HasColumnType("float");

                    b.Property<bool>("iptal")
                        .HasColumnType("bit");

                    b.Property<bool>("odendi")
                        .HasColumnType("bit");

                    b.HasKey("Id");

                    b.ToTable("taahütname");
                });

            modelBuilder.Entity("Entities.Models.Tatil", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool>("ApiMi")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime");

                    b.Property<DateTime>("Tarih")
                        .HasColumnType("datetime");

                    b.Property<string>("TatilAdi")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Tatil");
                });

            modelBuilder.Entity("Entities.Models.TedaviKaydi", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("MuyaneId")
                        .HasColumnType("int");

                    b.Property<bool>("Odendi")
                        .HasColumnType("bit");

                    b.Property<int>("doktorId")
                        .HasColumnType("int");

                    b.Property<double>("fiyat")
                        .HasColumnType("float");

                    b.Property<int>("prtokol")
                        .HasColumnType("int");

                    b.Property<string>("tedaviAdi")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("tedaviKodu")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("TedaviKaydi");
                });

            modelBuilder.Entity("Entities.Models.Tetkikler", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<double>("Fiyat")
                        .HasColumnType("float");

                    b.Property<string>("Kodu")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TetikAdi")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("aktifMi")
                        .HasColumnType("bit");

                    b.HasKey("Id");

                    b.ToTable("Tetkikler");
                });

            modelBuilder.Entity("Entities.Models.User", b =>
                {
                    b.Property<int>("UserID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("UserID"));

                    b.Property<string>("Email")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("RefreshToken")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<DateTime?>("RefreshTokenExpiry")
                        .HasColumnType("datetime");

                    b.Property<string>("Surname")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("UserName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("UserID");

                    b.ToTable("users", (string)null);

                    b.HasData(
                        new
                        {
                            UserID = 1,
                            Email = "xxxx@gmail.com",
                            Name = "BATUHAN",
                            Password = "12345",
                            Surname = "KÖSE",
                            UserName = "ADMIN"
                        });
                });

            modelBuilder.Entity("Entities.Models.UserLog", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("AksiyonTipi")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Detay")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("EntityId")
                        .HasColumnType("int");

                    b.Property<string>("EntityTipi")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("IpAdresi")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("OlusturmaTarihi")
                        .HasColumnType("datetime");

                    b.Property<int?>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("userLogs");
                });

            modelBuilder.Entity("Entities.Models.UserYetki", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<int>("YetkiId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("YetkiId");

                    b.HasIndex("UserId", "YetkiId")
                        .IsUnique();

                    b.ToTable("user_yetkiler", (string)null);
                });

            modelBuilder.Entity("Entities.Models.Yetki", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Ad")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Kod")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("Id");

                    b.HasIndex("Kod")
                        .IsUnique();

                    b.ToTable("yetkiler", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Ad = "Randevu Açma",
                            Kod = "randevu_acma"
                        },
                        new
                        {
                            Id = 2,
                            Ad = "Muayene Açma",
                            Kod = "muayene_acma"
                        },
                        new
                        {
                            Id = 3,
                            Ad = "Redis Atma",
                            Kod = "Redis_atma"
                        });
                });

            modelBuilder.Entity("Entities.Models.odeme", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("muayeneId")
                        .HasColumnType("int");

                    b.Property<DateTime>("odemeTarihi")
                        .HasColumnType("datetime");

                    b.Property<double>("odemeToplam")
                        .HasColumnType("float");

                    b.Property<int>("tedaviId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("odeme");
                });

            modelBuilder.Entity("Entities.Models.teshisler", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("muayeneId")
                        .HasColumnType("int");

                    b.Property<string>("teshisAd")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("teshisKod")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Teshisler");
                });

            modelBuilder.Entity("Entities.Models.UserYetki", b =>
                {
                    b.HasOne("Entities.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Entities.Models.Yetki", "Yetki")
                        .WithMany("UserYetkiler")
                        .HasForeignKey("YetkiId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");

                    b.Navigation("Yetki");
                });

            modelBuilder.Entity("Entities.Models.Yetki", b =>
                {
                    b.Navigation("UserYetkiler");
                });
#pragma warning restore 612, 618
        }
    }
}
